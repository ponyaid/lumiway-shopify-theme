...
function updateCart(data) {
    data.items.forEach(item => {
        ...
        cartItem.querySelector('.line-price').innerHTML = Shopify.formatMoney(item.line_price)
        cartItem.querySelector('.inp-qty').value = item.quantity
    })
    document.getElementById('cart-total-price').innerHTML = Shopify.formatMoney(data.total_price)
}
...
function updateCartDrawer(data) {
    ...
    if (data && cartDrawerTbody) {
        for (const item of data.items) {
            const row = `<div id="cart-drawer-${ item.id }" class="border-bottom" style="padding: var(--bs-offcanvas-padding-x) var(--bs-offcanvas-padding-y);">
                    <div class="row g-2">
                        <div class="col-3">
                        ...
                        <div class="col">
                            <div class="d-flex align-items-lg-start">
                                <a class="h6 m-0" href="${item.url}">${item.product_title}</a>
                            ...
                            <div class="d-flex">
                                ...
                                <div class="ms-auto">
                                    ${item.original_line_price != item.line_price ? `<span>${Shopify.formatMoney(item.original_line_price)}</span>` : ""}
                                    <span class="fw-semibold">${Shopify.formatMoney(item.line_price)}</span>
                                </div>
                            </div>
                        </div>
                    </div>
    ...
    document.getElementById('cart-drawer-total-price').innerHTML = Shopify.formatMoney(data.total_price)
    document.getElementById('cart-drawer-items-badge').innerHTML = data.item_count
}
